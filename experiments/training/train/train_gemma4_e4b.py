#!/usr/bin/env python3
"""
Projeto IANA — Treino do Gemma 4 E4B com LoRA para NER + SOAP.

v2: completion-only masking via DataCollatorForCompletionOnlyLM (TRL).
    Mesma estrutura do MedGemma. Gemma 4 E4B tem vision/audio towers que
    precisam ser removidas antes do LoRA (senão os targets q_proj/k_proj
    caem em tower em vez de language_model e grad_norm fica 0).

Uso:
    python train_gemma4_e4b.py --config ../config/gemma4_e4b.yaml --gpu 0
    python train_gemma4_e4b.py --dry-run
"""
...
def main():
    parser = argparse.ArgumentParser(description="IANA — Treino Gemma 4 E4B LoRA")
    parser.add_argument("--config", default=str(_TRAINING_DIR / "config" / "gemma4_e4b.yaml"))
    ...
    # Remove towers multimodais (text-only training).
    ...
    for attr in ("vision_tower", "audio_tower"):
        if hasattr(_inner, attr):
            ...
            print(json.dumps({"event": f"removed_{attr}"}))
    ...
    _od = Path(cfg.get("output_dir", "checkpoints/gemma4_e4b"))
    ...
    sft_config = SFTConfig(
        ...
        eval_accumulation_steps=cfg.get("eval_accumulation_steps", 4),
    ...
    json_cb = JSONLoggingCallback(
        log_path=log_dir / f"gemma4_e4b_{int(time.time())}.jsonl",
